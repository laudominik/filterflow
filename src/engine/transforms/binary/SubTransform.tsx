...
import BinaryPixelVisualizationComponent from "../../../components/visualizations/BinaryPixelVisualizationComponent";
...
@jsonObject({name:"SubtractionTransform"})
export default class SubtractionTransform extends BinaryTransform {
    constructor(){
        super("subtraction", fs);
    }
    ...
    public infoView(): string | null {
        return "Subtracts pixel values from for each channel: color1 - color2"
    }

    visualizationView(guid: string): JSX.Element {
        return <BinaryPixelVisualizationComponent guid={guid} operantName="-" op={(x,y) => x-y}/>
    }
}