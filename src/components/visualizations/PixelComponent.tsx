...
export function FloatPixelComponent(color: [number, number, number], channel: Channel){
    let styleColor: string;
    let value = 0;

    switch (channel) {
        case Channel.RED:
            styleColor =`rgba(${color[0]*255}, 0, 0, 255)`
            value = color[0]
            break;
        case Channel.GREEN: 
            styleColor = `rgba(0, ${color[1]*255}, 0, 255)`
            value = color[1]
            break;
        case Channel.BLUE: 
            styleColor = `rgba(0, 0, ${color[2]*255}, 255)`
            value = color[2]
            break;
        case Channel.GRAY: 
            styleColor = `rgba(${color[0]*255}, ${color[0]*255}, ${color[0]*255}, 255)`
            value = color[0]
            break;
        case Channel.NONE:
            styleColor = `white`
